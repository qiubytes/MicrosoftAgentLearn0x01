...
返回的JSON格式 包括  工作类别(worklog_type),分类原因(classify_reason)
                                ";
            return new ChatClientAgent(chatClient, new ChatClientAgentOptions(instructions: instructions)
            {
                ChatOptions = new()
                {
                    ResponseFormat = ChatResponseFormat.ForJsonSchema(AIJsonUtilities.CreateJsonSchema(typeof(WorkLogTypeDto)))
                }
            });
        }
        /// <summary>
        ...
        public static ChatClientAgent GetWorkLogRiskAgent(IChatClient chatClient)
        {
            string instructions = @" 
                                    ...
                                    返回的JSON格式 包括  风险等级(risk_level),风险原因(risk_reason)
                                ";
            return new ChatClientAgent(chatClient, new ChatClientAgentOptions(instructions: instructions)
            {
                ChatOptions = new()
                {
                    ResponseFormat = ChatResponseFormat.ForJsonSchema(AIJsonUtilities.CreateJsonSchema(typeof(WorkLogRiskDto)))
                }
            });
        }
